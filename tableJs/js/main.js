(function() {
    ...
    function createTable() {
        ...
        let ids = ['firstName', 'lastName', 'about', 'eyeColor'];
        let values = ['Имя', 'Фамилия', 'Описание', 'Цвет глаз'];
    ...
    function createRow(person) {
        ...
        firstName.textContent = person['name']['firstName'];
        lastName.textContent = person['name']['lastName'];
        about.textContent = person['about'];
        eyeColor.textContent = person['eyeColor'];
    ...
    // создание формы замены данных
    function createForm(tr, data) {

        // удаляем уже созданную форму если существует
        ...
        let person = data[data.findIndex(o => o.id == tr.id)];
        ...
        about.rows = '5';
        about.cols = '30';
        about.placeholder = 'Описание';
        about.classList.add('input');
        about.required = true;
        eyeColor.type = 'text';
        eyeColor.placeholder = 'Цвет глаз';
        eyeColor.classList.add('input');
        eyeColor.required = true;
        submitBtn.type = 'submit';
        submitBtn.value = 'Заменить';
        submitBtn.classList.add('input', 'btn');


        // добавление значений в input
        firstName.value = person['name']['firstName'];
        lastName.value = person['name']['lastName'];
        about.textContent = person['about'];
        eyeColor.value = person['eyeColor'];

        // Добавление блоков ввода в форму
        form.append(closeBtn);
        form.append(firstName);
        form.append(lastName);
        form.append(about);
        form.append(eyeColor);
        ...
        form.addEventListener('submit', (e) => {
            ...
            tr.children[0].textContent = firstName.value;
            tr.children[1].textContent = lastName.value;
            tr.children[2].textContent = about.value;
            tr.children[3].textContent = eyeColor.value;
            // обновление данных в массиве
            person['name']['firstName'] = firstName.value;
            person['name']['lastName'] = lastName.value;
            person['about'] = about.value;
            person['eyeColor'] = eyeColor.value;
            
            form.remove();
        });
    }
    
    // добавление таблицы на страницу и создание общей логики
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // добавляем данные в tbody и далее в таблицу
        for (let person of data) {
            let tr = createRow(person);
        ...
        headings.forEach(heading => heading.addEventListener('click', () => {
            Array.from(tbody.querySelectorAll('tr:nth-child(n+1)'))
                .sort(comparer(Array.from(heading.parentNode.children).indexOf(heading), this.asc = !this.asc))
                .forEach(tr => tbody.appendChild(tr) );
        }));

        // добавление формы при клике на строку
        let trs = tbody.querySelectorAll('tr');

        trs.forEach(tr => tr.addEventListener('click', () => createForm(tr, data)));
